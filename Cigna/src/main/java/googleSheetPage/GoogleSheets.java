...
public class GoogleSheets extends CommonAPI{
    ...
    public List<String> searchItemByName(String spreadsheetId, String range) throws IOException, InterruptedException {
        ...
        for (List row : values) {
            Thread.sleep(2000);
            keysInput(".//*[@id='csng-search-header']/search-header-bar/form/cigna-typeahead/div/input");
            actual.add(getTextByXpath("//*[text()='"+row.get(1).toString()+"']"));
            Thread.sleep(2000);
            driver.findElement(By.id(".//*[@id='csng-search-header']/search-header-bar/form/cigna-typeahead/div/input")).sendKeys(Keys.chord(Keys.CONTROL, "a"),Keys.BACK_SPACE);
